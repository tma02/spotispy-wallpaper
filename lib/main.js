...
function createWindow () {
  ...
  mainWindow = new BrowserWindow({width: 1366, height: 790, frame: false, skipTaskbar: true});
  ...
  mainWindow.setBounds({ x: 10000, y: 10000, width: 1920, height: 1080 });

  ipc.on('coverUrl', function(event, arg) {
    if (arg) {
      mainWindow.capturePage(function(image) {
        fs.writeFile('temp.png', image.toPng(), function (err) {
          if (err) return console.log(err);
          wallpaper.set('temp.png').then(() => {
  ...
  var contextMenu = Menu.buildFromTemplate([
    { label: 'Exit Spotispy', click: function() { mainWindow.close(); } }
  ]);
